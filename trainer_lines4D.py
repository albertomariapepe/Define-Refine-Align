...
class G4LinesTrainer:
  ...
  def _train_epoch(self, epoch):
    ...
    for curr_iter in range(len(data_loader) // iter_size):
      ...
      for iter_idx in range(iter_size):
        ...
        matches, line_bundle1, line_bundle2, M_gt, R_gt, t_gt = next(data_loader_iter)
        ...
        RegressionLoss = MotorLoss().to(self.device)
        loss = RegressionLoss(M_gt, M)
  ...
  def _valid_epoch(self):
    ...
    for batch_idx in range(tot_num_data):
      ...
      '''   
      err_q, err_t = self.evaluate_R_t(R_gt[0,:,:].numpy(), t_gt.numpy(), M.detach().cpu().numpy())
  ...
  def evaluate_R_t(self, R_gt, t_gt, M, q_gt=None):
    
    print(M, flush = True)
    R_est, t_est = project_G4_to_G3(M)
